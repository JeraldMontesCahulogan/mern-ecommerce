...
const GiftCouponCard = () => {
  ...
  const { coupon, isCouponApplied, applyCoupon, getMyCoupon, removeCoupon } = useCartStore();
	...
	// Fetch the user's coupon in first run
	useEffect(() => {
		getMyCoupon();
	}, [getMyCoupon]);

	// if we have a coupon, set the user input code to the coupon code
	useEffect(() => {
		if (coupon) setUserInputCode(coupon.code);
	}, [coupon]);

	// if the coupon is applied, set the user input code to the coupon code
	// if no coupon, return
  ...
  return (
    <motion.div
      ...
      {/* if we have a coupon and it is applied, do the following */}
      ...
      {/* if we have a coupon and it is not applied, do the following */}
      {coupon && (
				<div className='mt-4'>
